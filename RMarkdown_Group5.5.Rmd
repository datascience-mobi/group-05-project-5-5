...
```{r}
heatmap(
  as.matrix(cor(t(clustering_data))), 
...
After that we still had correlation which was not optimal but we faced too much data loss if we would have tried to procede in reducing more.
...
summ(regression_model_uncorrelated_genes, confint = TRUE, digits = 3, ci.width = .9)
```
...
```{r warning=FALSE}
predict(regression_model_uncorrelated_genes, newdata = regression_data_uncorrelated_genes, type = "response")
```
...
predict(regression_model_cross_validation, newdata = test.set, type = "response")
...
```{r}
regression_data_all_genes_before_uncorrelation <- cbind(
...
As previous mentioned, this model is as bad as it possibly can be. We faced with alot of NA values (only 10 variables with coefficients) based on highly correlated data on the one and on the other hand we had to deal with values of approximately 1. We rejected this model and did some extra work to create a better model shown above.
...
regression_data_researched_important_genes <- cbind(Health_status = as.numeric(c(0,
                                                      0,
                                                      0,
                                                      0,
                                                      0,
...
predict(regression_model_researched_important_genes, newdata = regression_data_researched_important_genes, type = "response")
```
...
# Conclusion
As a final conclusion, we faced different problems through this downstream analysis and at the end we were not fully able to create a full functional and good regression model to predict MCL in given samples. Our previous researched genes cannot be confirmed as different methylated in gene coding region. But we did not test for different methylated regions (DMRs) in other coding and non-coding regions of our samples. It could be possible that these genes are DMRs e.g. in promotor region.</div>
$$~$$
