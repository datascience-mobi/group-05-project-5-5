...
In order to visualise our kmeans clutering data we also added an extra column with the indication healthy or cancer to `centers` data-set so that we can later color code the healthy and cancer patinets. 
...
For each of those we needed to obtain a p-value in order to assess it's sigificance, thus we created a matrix for those p-values in order to later visualise our results with a heatmap. The following code shows the backbone of that matrix.
...
We used Kruskal-Wallis Rank Sum Test for to test the significane of submission date, because we have more than two possible and independent conditions (Rasch et al., 2010).
Monte Carlo permutation test /calculation of the correlation coefficient with a following permitation test
donor age and sequence run counts
...
p_cluster_disease
```
It is clearly visible that the clustering did not work out very well because of the bach effects. As indicated by the plots in the cell type plot is only one healty cell type in its own cluster and the other 4 Bcells are together with the cancer cells in a cluster. 
The same 
$$~$$
