...
We included mean and standard deviation values to the original data frame, therefore, we can later visualize them. Additionally, we created a data frame which only include our cancer related genes. This data frame is extended by gene name rather than ensembl name.
```{r}
...
ggplot() +
  ...
  ylim(-12, 9) 
```
After that we focused on details. The following detailed plot visualizes:
