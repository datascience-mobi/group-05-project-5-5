...
Theoretically we there should be only the two groups cancer and healthy. But we obtained 3 groups. This is because of batch effects. One group are the cancer samples. The second group are healthy samples from Jose I. Martin-Subero who is one of the biomaterial providers. The third group are also healthy samples but they are from NIIHR Cambridge BioResource. The probleme here is that we have distinct values in the cancer samples. But we do not know how the batch effects influence the healthy samples in detail. The only thing we know is that they are influenced. So we can not delete the batch effects.
...
The two plots from disease and cell type look alike because the data are the same. B-cells are always healthy in our dataset and cause no cancer. In comparison to that cancer cells are always from the cancer type mantle cell lymphoma in our dataset.
