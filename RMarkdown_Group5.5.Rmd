...
Our data from MCL patients and a healthy control group originate from a project called BLUEPRINT (Blueprint Project). It was a research association from 2011 to 2016 with the goal to understand the gene activation and gene expression better (Blueprint Project). To do so they generated reference epigenomes (Blueprint Project). They focused on certain haematopoietic cells from healthy and leukaemic people, but also on some auto immune diseases (Blueprint Project). 

Epigenetic modifications have an impact on DNA expression (Gibney and Nolan, 2010) and are important in some tumor genesis (Horn et al., 2009). These modifications are for example DNA methylation, histone modification and RNA mediated processes (Gibney and Nolan, 2010). In our project we want to focus on DNA methylation. In humans only cytosine can be methylated to 5-mehtylcytosine (5mC) (Walter and Huempel, 2017). In order for methylation to be inherited it needs to be in a CpG dimer, because in such a dimer there is a cytosine in the mother and daughter strand (Horn et al., 2009). After the DNA replication the enzyme DNA-methyltransferase 1 is able to methylate the daughter stand if there is a methylation in the CpG of the mother strand (Horn et al., 2009). During the lifetime of a cell different genes need to be expressed and therefore methylated. DNA methyltransferase 3a and 3b are able to methylate cytosine although there is no methylation in the complementary strand (Horn et al., 2009).
Methylation in promoters normally leads to inactivation of the following gene (Horn et al., 2009). But there is also the possibility of activating a gene by methylation (Yang et al., 2014). For this the methylation needs to be in the gene.

For an analysis of DNA methylation more steps are required than in normal DNA-sequencing because "normal" cytosine and 5-mC need to be distinguished. The methylation pattern of the samples in our data set was analyzed by the bisulfite sequencing method (bisulfite seq). The chemical sodium bisulfite converts unmethylated cytosine into uracil (Bock, 2012). The DNA with the converted bases can be amplified by PCR and after that it can be analysed by a next generation sequencing method (Bock, 2012). In our case they used Illumina. After the analysis of the converted and unconverted DNA both sequences can be compared (Bock, 2012). If thymine pairs with cytosine an unmethylated cytosine was in this position in the original DNA (Bock, 2012). Thymine which pairs with adenine were originally thymine and every cytosine in the bisulfite treated DNA stands for a 5-mC (Bock, 2012). One problem of bisulife seq is that bisulfite can not distinguish between 5-mC and 5-hydroxymethylcytosine (5-hmC) (Huang et al., 2010). So it is not clear whether every methylated cytosine in our data set is really a 5-mC or not (Huang et al., 2010). The problem with this is that it has been observed that 5-hmC has not the same effect on gene expression as 5-mC (Moen et al., 2015). Advantages of bisulfite seq are great reproducibility, high accuracy and a comprehensive genomic coverage (Bock, 2012).
$$~$$
...
Split data into 4 data frames (cancer vs. control, beta-values and coverage) $$~$$ | Add a column for mean, median and SD for better comparisons | Identify batch effect magnitude in the data set | Apply logistic regression |   |
Set a threshold and examine variance for reliable coverage, replace unreliable beta-values with NA $$~$$ | Visualization through plots | Apply clustering method and visualize, e.g k-means |  |  |
Separately inspect genes of interest for MCL $$~$$ |   |    |    |   
...
$$~$$
For now, we want to single out the gene data frame of the patients to inspect it separately. We will examine only the gene data frame and neglect the data from tiling, promoters and cpg-islands because we want to focus on the genes in our project even though the other genome parts are important too for a comprehensive analysis.
...
There is much information from 56175 genes across all 10 patients. We are mostly interested in the coverage and beta values. After the entire data clean-up we still want to retain around 90% of our beta value information (~500000 genes).

We are starting the data clean-up with the sex chromosomes. In our case we have different numbers of female/male patients in healthy/cancer sample groups. In females X chromosome inactivation leads to the silencing of the majority of genes on one X, this would lead to significant differences in the methylation profiles between female/male patients (Sharp et al., 2011). Therefore we removed gene data of sex chromosomes. 
...
Then we created a histogram with mean coverage values for each gene on a common log scale and added lines for the 10 quantiles from 0 % to 100 %.
...
It was essential that we used a common logarithm scale because our coverages were widely spread over a great range of values. But it is easy to see that we have two groups of mean coverage values in healthy coverage histogram. In cancer coverage histogram it was harder to decide how to handle the mean values between 10 and 100 coverages. We concluded that we also had two major mean value groups. But overall it is seen that the majority of our data have a high coverage value and that we could later adapt our analysis to it. Not even one fifth of the beta values had less than 100 coverages. As previously said, healthy beta values had stronger coverages than cancer beta values. We need to see if this will effect our downstream analysis.

To check how widely spread the coverages across the patients are, we calculated the standard deviations (SD) of each gene across 10 patients. We plotted the SD-values for cancer and healthy samples on a histogram with a log scale with their corresponding quantiles from 0 % to 100 %.
...
cancer_coverage <-
  apply(cancer_coverage, MARGIN = c(1, 2), FUN = cancer_threshold_function)
...
healthy_coverage <-
  apply(healthy_coverage, MARGIN = c(1, 2), FUN = healthy_threshold_function)

healthy_beta_values[healthy_coverage == "NA"] <- NA
healthy_coverage[healthy_coverage == "NA"] <- NA
```

Now we needed to deal with NA's in the beta-values, because they represent no information and they could cause errors in our further analysis. We had many NA's but we cannot remove every gene with a NA because this would lead to a loss of information.
Our sum of NA's is:
...
To visualize the number of NA's in the beta-values we plotted several histograms.
...
The first histogram of cancer/healthy is to get an overview over all numbers of missing values per gene. Most genes in `cancer_beta_values` and `healthy_beta_values` fortunately have no NA's.
We wanted to see the differences between 2, 3, 4 and 5 NA's per gene better. To do so we reduced the range of the axis. Now it is visible that in `healty_beta_values` around 800 genes have 5 NA's. With 3 or 4 NA's there are respectively a bit less than 1500 genes. In comparison to that `cancer_beta_values` has around 1200 genes with 5 NA's but less genes with 2, 3 or 4 NA's. 

Then we removed the genes with 3 or more missing values. Here it was important to remove the genes in both data frames, because a gene in only one sample group without values to compare to is meaningless. After the deletion we checked if the rownames are the same in both data frames and it was the case. This is important because otherwise unequal amount of rows between the two data frames would lead to problems in the downstream analysis. 
...
We removed the columns with the NA-values because we did not need them anymore. 
...
To cut things short and to make it a little easier for our next analysis, we transposed both beta value data frames (cancer and healthy) for the replacement. Afterwards we transposed both data frames back for further analysis.
...
We checked that there are no more NA's left in the cancer and healthy data frames.
...
```
$$~$$
There are several important genes for MCL which have often a different methylation pattern in comparison to heatlhy B-cells. These genes are:
...
To be sure we did not remove these genes in our data processing we tested if they are still in our data frames. As each of the 2 data frames contains the same genes it is enough to check if these important genes are in one of them.
...
We can continue with our next step: data normalization and visualization.
$$~$$
...
Compared to literature, beta-value to m-value transition worked perfectly and we can continue our downstream analysis (Du et al., 2010). Aberration of transition occured because of mean value calculation.

Afterwards we changed the endings of the patinets names to make it more clear to which data frame they belonged to.
...
Thereafter we calculated the means and standard deviations of m values, subsequently plotting a histogram for each, to see how they are spread.
...
Comparing the mean frequency histograms of cancer and healthy samples, we can conclude that both are right-skewed, whereas in healthy m values the effect is more prominent. In the mean m value histogram of healthy patients we can observe that the median lays around 0.6 which corresponds to beta values of around 0.8, in comparison to cancer, where the median lays around 0.2 which corresponds to beta values of 0.6. Important to mention is the fact that there are far less genes which have low beta values in the healthy samples than in the cancer samples (judging by the tenth quantiles).
But overall we can assume a that the distribution is normal in both cancer and healthy histograms.
Furthermore, we created a data frame which only includes our cancer related genes. This data frame is extended by gene name rather than ensembl name.
...
* general trend of points to solve overplotting problem (gray smoothed curve),
* straight line through the origin (y=x) for better interpretation (yellow line).
...
For better plot evaluation we drew a line with the equation y = x. Thus, we can identify if there is a linear relationship. Overall, our predicted important genes have not fully lost their methylation or gained full methylation. 
...
* possible effects on the rates of transcript elongation and splicing (Yang et al., 2014).

Therefore, we can only predict what otucome differentially methylated gene regions could have.
$$~$$
...
To validate our assumptions it would be important to analyse protein concentrations in cancer cells and compare it to DNA methylation status of genes.

For better evaluation of how conclusive following results are, it will be helpfull, if we compare standard deviations (SD) of cancer and healthy m-values.
...
Our standard deviations are generally not that high and only a few genes have high standard deviations.
...
In general, our important genes for MCL have a low standard deviation which enhances the explanatory power of our previous assumptions. A few genes tend to be more differently methylated in cancer samples than in healthy samples. For example one of them is SOX11.
SOX11 tends to be differently methylated in cancer samples. This observation can be made because MCL can be devided into 2 groups: SOX11 positive and SOX11 negative patients (Ferrando, 2013). Depending on this status, different clinical outcomes can be observed (Ferrando, 2013).
In general, methylation of cancer patients have stronger discrepancies among each other because cancer origin is unique for each patient.
$$~$$

## Part 3: Data reduction

$$~$$
The aim of this part of the project is to narrow down our data to the significant genes which account for the majority of the variation between methylation values of healthy and cancer samples. Therefore, we need to reduce high dimensional data into low dimensional data with as little information loss as possible. 
...
These new principal components (features) can now be used for predictive analysis and model building. A crucial benefit is, that each principal component is independent of the other.
...
From the summary of the PCA we can conclude that PC1 and PC2 together account for more than 50% of the varaince in the data. To get a feeling of how the samples were spread across PC1 and PC2 we plotted a scatterplot. Additionally, we used the function `plotly()` to define each point on the plot by pointing on it.
...
The scatterplot visualzies that the methylation differences in healthy an cancer samples are well represented in PC1. But PC2 shows no separation of the two groups. This might be caused by batch effects which are an unwanted source of variation in data (Leek, 2012). To see if the batch effects are really the cause of the bad separation we carried out several statistical test.

But before doing so we need to verify a few assumptions we made. First we can perform kmeans clustering. We know that there should be two clusters (healthy/cancer) but to prove that, we plotted the number of clusteres against the total within-clusters sum of squares.
...
cluster <-
  kmeans(x = pcs_of_m_values[1:10],
         centers = 2,
         iter.max = 100)$cluster
```

The knick in the elbow is as expected at 2 clusters. This means we only have the two groups: cancer and healthy.

With the help of the elbow method we wanted to visualize what was said before. The first two PCs contain most information about the methylation differences.
...
The knick in the graph is clearly at the 2nd PC. Therefore we will be testing each batch effect in the first two principal components because they account for the majority of the variation in the data. 
...
For each of those we needed to obtain a p-value in order to assess its sigificance, thus we created a matrix for those p-values in order to later visualize our results with a heatmap. The following code shows the backbone of that matrix.
...
To check if there are significant differences in our data we created a new data frame with the data from the first two PCs. After that we added the specific columns from `input_data_csv` which we wanted to test on significant differences in the mehtylation pattern.
...
To be able to perform tests to find the p-values out it is important to convert the data from PC 1 and 2 from the class factor to numeric.
...
As the criteria which we are testing for possible significant differences have differnt formats, we need to apply appropriate statistical test for each of them. The statistical test which we chose are: Wilcoxon Rank Sum Test, Kruskal-Wallis Rank Sum Test and Monte Carlo permutation test.
...
Monte Carlo permutation test is a randomization test which can be used if the data is not normal distributed or if the distribution is not known (MA, Jianqiang, 2009). An advantage is the possibility to use the test on very big data sets (MA, Jianqiang, 2009). We decided to use this test on donor age and sequence run counts.
...
Here we filled the `p_values_matrix` with the corresponding test statistics p-values. A "for-loop" was used in order to avoid redundantly performing the same test on PC1 and PC2.
...
As expected the criteria of "disease" and "cell type" came out as significant which is a biological effect rather than a batch one. The only batch effect turned out to be the biomaterial type which is in PC1. Although the criteria of disease and cell type is only significant in PC1 we still decided to continue working with PC2 as it is completely batch-effect-free even though it is not ideal.

Additionally, we decided to visualize our biological/batch effects on scatter plot in order to see how they are distributed.
...
The batch effect in PC1 in biomaterial type causes that some samples from healthy and cancer are closer to each other than to other samples of the same group. The biomaterial type indicates if the samples derive from primary cells or primary tissue. Batch effects in this category mean that there are significant methylation differences within primary cells or primary tissue even though there should be no big differences. We only know that there are batch effects which influence our data but we can not delete the batch effects.
...
The two plots from disease and cell type look alike because the data are the same. B-cells are always healthy in our dataset and cause no cancer. In comparison to that cancer cells are always from the cancer type mantle cell lymphoma in our dataset. It is clearly visible that PC2 can not distinguish between healthy and cancer samples while PC1 can. Because of that fact there is a vertical seperation between the groups but no horizontal separation.
...
$$~$$
With this part we intended to see if our samples coincide with their labels (healthy/cancer) and find the genes that are the most differentially methylated between our healthy and cancer samples (differentially methylated regions (DMRs)). DMRs can help distinguish cancer from healthy patients and are therefore the basis of our logistic regression that will be used for predictions. We wanted to obtain the DMRs through a process with multiple steps. 

To begin with, we looked at the loading scores of the genes from PC2. These values can be used to find the genes that influenced our PCA the most by ranking the scores by their absolute value. Then we plotted the loadings to find a kink in the curve which indicates up until what gene the influence is significant. 
...
The kink is somewhere between 0 and 10000 genes so we adjusted the x-axis to have a better look at the kink.
...
Although the kink seems to be around 2000 genes when we tried to apply k means clustering with these 2000 genes the healthy and cancer samples weren't assigned to their respective clusters.

```{r}
set.seed(200)
top_2000_genes <- data.frame(ranked_gene_loading[1:2000])
top_2000_kmeans <-
  rbind(m_values[c(as.list.data.frame(rownames(top_2000_genes))), ])
km <-
  kmeans(x = t(top_2000_kmeans),
         centers = 2,
         iter.max = 500)
km$cluster
```

If we used only 2000 genes for our downstream analysis there wouldn't be reliable results coming out of it. To set the right initial conditions we tested how high the number of genes needs to be for the samples to be classified correctly. For this we applied k-means clustering for 1000 - 10000 genes and observed when the clusters were reliably right. 
First, we generated an empty matrix with the right dimension to put the cluster assignments of the samples in.

```{r}
clustering_matrix <- matrix(nrow = 10, ncol = 10)
...
To test the right amount of genes a loop goes through the first thousand to ten thousand genes with the highest loading scores and puts the allocated clusters into the prepared matrix.

```{r}
set.seed(200)
for (i in 1:10) {
  top_genes <- data.frame(ranked_gene_loading[1:(i * 1000)])
  clustering_data <-
    rbind(m_values[c(as.list.data.frame(rownames(top_genes))), ])
  k <-
    kmeans(
      x = t(clustering_data),
      centers = 2,
...
After generating this multiple times it became clear that from 5000 genes on the samples were assigned correctly. So, the top 5000 genes were used for further analysis.

```{r}
top_5000_genes <- data.frame(ranked_gene_loading[1:5000])
clustering_data <-
  rbind(m_values[c(as.list.data.frame(rownames(top_5000_genes))),])

```

Also we wanted to check if we used the amount of clusters that is also recommended by k means for our clustering. Because the purpose of our clustering is to review the right assignment to the sample label it would make sense to use two. We wanted to check if our chosen amount of data suggest this too.
...
plot(
  ...
  type = "b",
  pch = 19,
  main = "Elbow method for different numbers of clusters",
  xlab = "Number of clusters K",
  ylab = "Total within-clusters sum of squares"
)
```

By looking at the kink in the curve we found that as expected 2 cluster were the most suitable number of clusters for our data and thus, the clustering was correctly done. 

To find the differentially methylated regions we had to find the genes that were the most significantly different in cancer and healthy samples.
The significance of the difference for each gene was evaluated by measuring the p-value through the student's t test.
...
We also looked specifically at the p-values of the important genes for MCL we found in literature and whether the difference of these genes is significant in our data, too.

```{r}
clust_data_important_genes <-
  cbind(transposed_clustering_data[, c(unlist(important_genes))])
...
p_value_important_genes <-
  as.data.frame(cbind(important_genes, p_value_important_genes))
colnames(p_value_important_genes) <- c("Genes", "p-values")

p_value_important_genes

```
There is only one gene which p value is under 0.01, LGALS3 (ENSG00000131981) with p value = 0.0019. As in Part 1: Data Analysis already mentioned the function of LGALS3 is to elevate inflammatory processes. LGALS3 protects cancer cells by inducing tolerance to drug treatment.

To visualise our p-values and the distribution of the significance of our genes we generated a volcano plot. For the volcano plot the fold change is plotted along the x-axis. The fold change is calculated by applying log2 on the mean beta value of healthy and cancer samples respectively and then subtracting the cancer value from the healthy one. It is important to use beta values because these range only from 0 to 1 while m values can be negative figures of which log2 is not defined.
Along the y-axis the negative common logarithm of the p values is plotted. 
As thresholds we decided on 0.01 for p-values and 2.0 for the fold change which we visualized through ablines on the graph.

```{r}
top_genes_beta_values <-
  rbind(healthy_beta_values[c(as.list.data.frame(rownames(top_5000_genes))),])
top_genes_beta_values <-
  cbind(top_genes_beta_values, cancer_beta_values[c(as.list.data.frame(rownames(top_5000_genes))),])
top_genes_beta_values <-
  cbind(
    top_genes_beta_values,
    Mean_Healthy = log2(rowMeans(top_genes_beta_values[, 1:5])),
    Mean_Cancer = log2(rowMeans(top_genes_beta_values[, 6:10]))
  )

fold <-
  top_genes_beta_values$Mean_Healthy - top_genes_beta_values$Mean_Cancer
p_value <- p_value_each_gene[, 1]
...
abline(v = fold_threshold, col = "blue", lwd = 2)
abline(v = -fold_threshold,
       col = "red",
       lwd = 2)
abline(h = -log10(pvalue_threshold),
       col = "green",
       lwd = 2)

fold_filtered = abs(fold) >= fold_threshold
pvalue_filtered = abs(p_value) <= pvalue_threshold
filter_combined = fold_filtered & pvalue_filtered
...
points (fold[filter_combined &
               fold < 0],-log10(p_value[filter_combined &
                                          fold < 0]),
        pch = 20, col = "red")
points (fold[filter_combined &
               fold > 0],-log10(p_value[filter_combined &
...
The statistically significant (p-value <= 0.01) genes with a higher fold than 1 are hypermethylated (Higher methylated in cancer than in healthy samples) and highlighted in blue.
The genes that have a p-value under 0.01 and have a lower fold than -1 are hypomethylated (less methylated in cancer than in healthy samples). These are accentuated in red.
There are many genes with a fold change of around zero and many of the genes that seem to be biologically significant have a low statistical significance. This leads to only some genes with biological and statistical significance. Another interesting feature is that the genes in cancer samples tend to be more methylated than in healthy ones.

After having a first look at our genes plotted by significance we wanted to adjust the p values for multiple comparisons to avoid type I errors.
To find the best method for this we applied different correction methods and compared them.

```{r}
p_value_each_gene$BH <-
  p.adjust(p_value_each_gene$p_value_each_gene,
           method = "BH")
p_value_each_gene$bonferroni <-
  p.adjust(p_value_each_gene$p_value_each_gene,
           method = "bonferroni")
p_value_each_gene$holm <-
  p.adjust(p_value_each_gene$p_value_each_gene,
           method = "holm")
p_value_each_gene$hochberg <-
  p.adjust(p_value_each_gene$p_value_each_gene,
           method = "hochberg")
p_value_each_gene$hommel <-
  p.adjust(p_value_each_gene$p_value_each_gene,
           method = "hommel")
p_value_each_gene$BY <-
  p.adjust(p_value_each_gene$p_value_each_gene,
           method = "BY")
...
The adjustment of the p values with various methods is really diverse and dependent on the chosen method. The one that seems to be the most suitable for our data is the one of Bonferroni-Holm. After adjusting our p values we replaced the uncorrected ones in our data frame and decided on 0.05 as a threshold for them. 

```{r}
p_value_each_gene$Names <- rownames(clustering_data)

p_value_each_gene <-
  p_value_each_gene[which(p_value_each_gene$BH < 0.05), ]
...
To find the differentially methylated genes we used a similar approach to the one we used to make the volcano plot. 
Firstly, we calculated the mean m values of healthy and cancer samples and then calculated the absolute difference.

```{r}
healthy_mean_m_values_diff_methylated <-
  as.matrix(sapply(1:nrow(clustering_data), function(k) {
    rowMeans(clustering_data[k, 1:5])
  }))
cancer_mean_m_values_diff_methylated <-
  as.matrix(sapply(1:nrow(clustering_data), function(k) {
    rowMeans(clustering_data[k, 6:10])
  }))

mean_values_together <-
  as.matrix(
    cbind(
      cancer_mean_m_values_diff_methylated,
      healthy_mean_m_values_diff_methylated
...
absolute_diff_m_values <-
  as.matrix(sapply(1:nrow(mean_values_together), function(k) {
    abs(diff(mean_values_together[k,]))
  }))
```

Then, we set a threshold for the m value difference (>5) and the adjusted p values (<0.01) and searched for genes that meet the requirements as these are the ones that are the most significantly differentially methylated genes in our data set.

```{r}
p_value_each_gene <-
  as.data.frame(cbind(p_value_each_gene, absolute_diff_m_values))
diff_methylated_genes <-
  p_value_each_gene[(p_value_each_gene$BH < 0.01) &
                      (p_value_each_gene$absolute_diff_m_values > 5),]
dim(diff_methylated_genes)

```
With that, we obtained 75 DMRs.
...
$$~$$
To obtain a "good" prediction system, we had to check on correlation between our final genes first. A high correlation would result in a bad regression model because of insufficient information. Correlation ranges from 0 to 1. The value 0 means no correlation, therefore, maximum information. In contrast, the value 1 means high correlation, therefore, no information in our case. 
...
To conclude, there is high positive correlation between our samples within a group. But between our cancer and control/healthy groups we have almost no correlation. Hopefully, this is sufficient for our prediction model.

In the process of testing different models, we faced several problems. Firstly, we had way too many genes which were highly correlated. Thus, we tried to reduce the amount of genes with regard to the amount of their correlation. For this, we had to calculate their correlation index and set a threshold reasonably enough for further analysis.
...
correlation_top_genes <-
  correlation_top_genes[, !apply(correlation_top_genes,
                                 2, function(x) {
                                   any(x > 0.9 | x < -0.9)
                                 })]
...
Our chosen threshold was 0.9 because we did not want to be too conservative. Thus, each column (gene) which contained more than one value above this threshold was removed. Through this threshold we obtained a reasonably mediocre regression model which was better than nothing. Our correlation relationship significantly improved to our favour.

```{r}
correlation_top_genes <-
  cor(t(regression_data))
corrplot(correlation_top_genes)
```

After that we still had correlation which was not optimal but we would have faced too much data loss if we had tried to proceed in reducing more.
With this new data frame as a basis, we tried to fit a logistic regression model. We used a logistic regression model, because it manages to model a binomial outcome (0,1) with one or more explanatory variables. In our case the variables are the selected genes. It measures the relationship between the categorical dependent variable (cancer/healthy) and one or more independent variables by estimating probabilities using a logistic function which is the cumulative logistic distribution. 
Logistic function: 
$$ p(X) = \frac{e^{β_0+β_1X}}{1+e^{β_0+β_1X}} $$

```{r warning=FALSE}
regression_data_uncorrelated_genes <-
  cbind(Health_status =
          as.numeric(c(0,
                       0,
                       0,
...
regression_model_uncorrelated_genes <-
  glm(
    formula = Health_status ~ .,
    family = binomial(link = "logit"),
    data = regression_data_uncorrelated_genes,
    maxit = 4
  )

summary(regression_model_uncorrelated_genes)
summ(
  regression_model_uncorrelated_genes,
  confint = TRUE,
  digits = 3,
...
The error "did not converge" gave a first impression of our model. It is really bad. None of our remaining genes are significant as further predictors. Moreover, we obtained high standard errors which underline unperfect model creation. We had to reduce iterations to 4 to prevent error message "fitted probabilities numerically 0 or 1 occurred". This indicated that both data were seperated in two sides. On the one side (cancer(1)) and on the other side (healthy(0)). Without cross validation, we tried to test this model on the same data again. 
Moreover, we calculated the value of χ-squared of our model and its p-value. Because of sufficient data it seemed that our model is good enough (p = 0.055) and fitted our needs (if we set α-level to e.g. α = 0.05 and therfore not rejecting our H0-hypothesis). 
Furthermore, when analyzing data with a logistic regression, an equivalent statistic to R-squared does not exist. However, to evaluate the goodness-of-fit of logistic models, several pseudo R-squareds have been developed. These are “pseudo” R-squareds because they look like R-squared in the sense that they are on a similar scale, ranging from 0 to 1 (though some pseudo R-squareds never achieve 0 or 1) with higher values indicating better model fit, but they can not be interpreted as one would interpret an OLS R-squared and different pseudo R-squareds can appear at very different values. Our logistic regression model had pretty good pseudo R-squared values (0.997 and 0.994) with two different methods. Thus, we estimated that our model has a good goodness-of-fit.

```{r warning=FALSE}
predict(regression_model_uncorrelated_genes,
        newdata = regression_data_uncorrelated_genes,
        type = "response")
```
...
Here the cross validation can be seen:

```{r warning=FALSE}
control <-
  which(regression_data_uncorrelated_genes$Health_status == "0")
control.train <-
  sample(control, floor(0.75 * length(control)))
cancer <-
  which(regression_data_uncorrelated_genes$Health_status == "1")
cancer.train <-
  sample(cancer, floor(0.75 * length(cancer)))
train <-
  c(control.train, cancer.train)
train.set <-
  regression_data_uncorrelated_genes[train,]
test.set <-
  regression_data_uncorrelated_genes[-train,]

regression_model_cross_validation <-
  glm(
    formula = Health_status ~ .,
    family = binomial(link = "logit"),
    data = train.set,
...
predict(regression_model_cross_validation,
        newdata = test.set,
        type = "response")

summ(regression_model_cross_validation)
```

After splitting up the data into test and train set, we modelled a regression system based on the train set. Then we used this regression model to finally see if this model was good enough to predict the test set. Using this on the test set had mutiple false predictions thus samples were not predicted correctly. 
Overall χ-squared (p=0.14) and pseudo R-squared values improved slightly. This indicated an improved logistic regression model but we could not validate this because of the failed predictions.
...
Gen | Information
:-------:|:-----------------------------------------------------------------------------------:
ENSG00000243702 | RNA, 7SL, Cytoplasmic 638, Pseudogene: It is reportes to be a part of the signal recognition particle of the ribonucleoprotein complex. It is important for transfer of signal bearing protein into a membrane or transfer to endoplasmatic reticulum for secretion. (Safran et al., 2010)
ENSG00000253822 | Immunoglobulin Lambda Variable 3-24: Encodes for a part of the N-terminal variable region of the light chains in a immunoglobulin moleculare, which are the antigen recognition molecules of B cells.(Lefranc, 2014)(Safran et al., 2010)
ENSG00000238622 | Small Nucleolar RNA, C/D Box 97: The gene is from a group of non-coding RNAs. They are reported to have several roles in the cell, but one of the most prominent ones is functioning as guide RNAs in post-transcriptional synthesis of 2′-O-methylated nucleotides.(Vitali et al., 2003)(Safran et al., 2010)
ENSG00000207504 | RNA, U6 Small Nuclear 1031, Pseudogene: Have been proposed to be a part of the active center of the spliceosome.They function during the first and second step of the splicing process.(Hu et al., 2004)(Safran et al., 2010)
ENSG00000244433 | piggyBac transposable element derived 4 pseudogene 7: short inverted-repeat-type DNA transposable element which was oroginally found in a moth genome, but similar sequences have also been found in humans. Possible function as a transposase, which catalyses the movement of the transposon within the genome. (Sarkar et al., 2003)(Safran et al., 2010)
ENSG00000279415 | Uncategorized gene, and is affiliated with the lncRNA class.(Safran et al., 2010)
ENSG00000270336 | Protein Phosphatase 2, Catalytic Subunit, Beta Isozyme (PPP2CB) Pseudogene: Has been reported to code fo the catalytic subunit of theThis gene encodes the phosphatase 2A catalytic subunit. This phosphatase has been involved in the negative control of cell growth and division.(Safran et al., 2010) (Götz, J., 1998)

After all those analyse steps, we can conclude that the data, on which our analysis is based, is not proficient enough for our analysis. We can in no way guarantee that these genes are relevant for mantle cell lymphoma.
...
```{r}
regression_data_all_genes_before_uncorrelation <-
  cbind(Health_status = as.numeric(c(0,
                                     0,
                                     0,
                                     0,
                                     0,
                                     ...
                                     1,
                                     1,
                                     1)), as.data.frame(t(clustering_data)))
regression_model_all_genes_after_ttest <-
  glm(formula = Health_status ~ .,
      family = binomial(link = "logit"),
      data = regression_data_all_genes_before_uncorrelation)

summary(regression_model_all_genes_after_ttest)$coefficients
```

As previously mentioned, this model is as bad as it possibly can be. We faced a lot of NA values (only 10 variables with coefficients) based on highly correlated data on the one hand and on the other we had to deal with values of approximately 1. We rejected this model and did some extra work to create a better model shown above.
...
regression_data_researched_important_genes <-
  cbind(Health_status = as.numeric(c(0,
                                     0,
                                     0,
                                     0,
...
regression_model_researched_important_genes <-
  glm(
    formula = Health_status ~ .,
    family = binomial(link = "logit"),
    data = regression_data_researched_important_genes,
    maxit = 4
  )

summary(regression_model_researched_important_genes)
predict(regression_model_researched_important_genes,
        newdata = regression_data_researched_important_genes,
        type = "response")
```

This summary showed us, that we could not generate a regression model with this data. Again we got a lot of NA's and a model which seems to predict correctly but this was not true as can be seen in the summary.

# Conclusion
As a final conclusion, we faced different problems throughout this analysis and at the end we were not fully able to create a completely functional and good regression model to predict MCL in given samples. Our previous researched genes can not be confirmed as differentially methylated in gene coding regions. But we did not test for differentially methylated regions (DMRs) in other coding and non-coding genome regions of our samples. It could be possible that these genes are DMRs e.g. in promotor region.</div>
$$~$$
...
Encodeproject.org. (2019). Audits – ENCODE. [online] Available at: https://www.encodeproject.org/data-standards/audits/ [Accessed 20 Jul. 2019].	
...
Fei, F., Joo, E.J., Tarighat, S.S., Schiffer, I., Paz, H., Fabbri, M., Abdel-Azim, H., Groffen, J., and Heisterkamp, N. (2015). 

B-cell precursor acute lymphoblastic leukemia and stromal cells communicate through Galectin-3. Oncotarget 6, 11378-11394.	

Ferrando, A.A. (2013). SOX11 is a mantle cell lymphoma oncogene. Blood 121, 2169.	
...
Götz, J., Probst, A., Ehler, E., Hemmings, B., & Kues, W. (1998). Delayed embryonic lethality in mice lacking protein phosphatase 2A catalytic subunit Cα. Proceedings of the National Academy of Sciences, 95(21), 12370-12375.	
