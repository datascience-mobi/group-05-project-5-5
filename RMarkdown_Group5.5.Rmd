...
We decided that we want to remove each gene with 3 or more NA's in 'healthy_beta_values' or 'cancer_beta_values', because if there are not enough values our results will be not meaningful. To do so we added a new column with the number of NA's in each gene. 
...
We wanted to see the differences between 2 or more NA's per gene better. To do so we reduced the range of the axis. Now it is visible that in 'healty_beta_values' a little amount of genes have 5 NA's. In comparison to that 'cancer_beta_values' have more genes with 5 NA's but less genes with 2, 3 or 4 NA's.
$$~$$

Then we removed the genes with to much missing values. Here it was important to remove the genes in both data frames, because a gene in only one data frame without values to compare is meaningless. So we checked if the rownames are the same in both data frames after the deletion and it was true.

```{r}
healthy_beta_values_2 <- data.frame()
cancer_beta_values_2 <- data.frame()
healthy_beta_values_2 <-
  healthy_beta_values[!(healthy_beta_values$Number_of_NA > 2),]
cancer_beta_values_2 <-
  cancer_beta_values_2[!(cancer_beta_values$Number_of_NA > 2),]

cancer_beta_values_2 <-
  cancer_beta_values_2[!(healthy_beta_values$Number_of_NA > 2),]
healthy_beta_values_2 <-
  healthy_beta_values_2[!(cancer_beta_values$Number_of_NA > 2),]

cancer_beta_values <- cancer_beta_values_2
healthy_beta_values <- healthy_beta_values_2
```
