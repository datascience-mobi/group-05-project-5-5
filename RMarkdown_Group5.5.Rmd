...
If we used only 2000 genes for our downstream analysis no reliable results would come out of it. To set the right initial conditions we tested how high the number of genes needs to be for the samples to be classified correctly. For this we applied k-means clustering for 1000 - 10000 genes and observed when the clusters were reliably right. 
...
To test the right number of genes a loop goes through the first thousands of genes with the highest loading scores and puts the clusters into the prepared matrix.
...
After generating this multiple times it became clear that from 5000 genes on the samples are assigned correctly. So, the top 5000 genes are used for further analysis.
...
We used the elbow method for this approach.
...
By looking at the kink in the curve we found that as expected 2 clusters is the most suitable number of clusters for our data. 

The significance of the difference between healthy and cancer samples for each gene was evaluated by measuring the p-value through the student t test.
...
As thresholds we decided on 0.01 for p-values and 1.0 for the fold change which we visualized through ablines on the graph.
...
points (fold[filter_combined & fold < 0],-log10(p_value[filter_combined &
                                                          fold < 0]),
        pch = 20, col = "red")
points (fold[filter_combined & fold > 0],-log10(p_value[filter_combined &
                                                          fold > 0]),
...
The adjustment of the p values with various methods are really diverse. The one that seems to be the most suitable for our data is the one of Bonferroni-Holm. After adjusting our p values we replaced the uncorrected ones in our data frame and decided on 0.05 as a threshold for them. 
...
```{r}
healthy_mean_m_values_diff_methylated <- as.matrix(sapply(1:nrow(clustering_data), function(k) {
  rowMeans(clustering_data[k, 1:5])
}))
cancer_mean_m_values_diff_methylated <- as.matrix(sapply(1:nrow(clustering_data), function(k) {
...
absolute_diff_m_values <- as.matrix(sapply(1:nrow(mean_values_together), function(k) {
