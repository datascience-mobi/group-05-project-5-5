...
But first we need to twick our data. There are several +/- inf values in our m-values dataset, with which PCA doesn't function. These were cause by beta values of 0 and 1. The most straightforward solution would be to approximate these values with 0.00001 and 0.99999.
...
Now, we want to find out the number of PC's we want to work with using the elbow method. 
For that we need to calculate the proportion of variance "by hand" and plot that against the number of PC's.
...
To get a feel of how the samples were spread across PC1 and PC2 we plottet a scatterplot using `ggplot()`. Additionally we used the function plotly()` to define each point on the plot by pointing on it.
