...
For now we want to single out the gene data frame of the patients to inspect it separately. We will examine only the gene data frame and neglect the datas from tiling, promoters and cpg-islands because they are irrelevant to answer our research question.
...
?Erfolg der Datenbereinigung mit Histogrammen belegen/veranschaulichen?

```{r}

```

Now we needed to deal with NA's in the beta-values. We decided that we want to remove each gene with 3 or more NA's in 'healthy_beta_values' or 'cancer_beta_values', because if there are not enough values our results will be not meaningfull. To do so we added a new column with the number of NA's in each gene. 
...
Then we removed the genes with to much missing values. Here it was important to remove the genes in both data frames, because a gene in only one data frame wihout values to compare is meaningless.
