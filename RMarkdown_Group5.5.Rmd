...
There is much information from 56175 genes across all 10 patients. Mostly we are interested in the coverage and beta values. After the entire data clean-up we still want to retain around 90% of our beta value information (~500000 genes).
...
The knick in the elbow is as expected at 2 clusters. This means we have really only the two groups cancer and healthy.

Wiht the following plot we wanted to visualize what was said before. The first two PCs contain most information about the methylation differences.

###### L�schen?      We need to identify which PC has many batch effects so that we can neglcets it further down the line of our analysis. We can do that using the elbow method. For that we need to calculate the proportion of variance "by hand" and plot that against the number of PC's.
...
Here we filled the `p_values_matrix` with the corresponding test statistics p-values. "for-loop" was used in order to avoid redundantly performing the same test on PC1 and PC2.
...
As expected the criteria of "disease" and "cell type" came out as significant which is a biological effect rather a batch one. The only batch effect turned out to be the biomaterial provider which is in PC1. Although the criteria of disease and cell type is only significant in PC1 we still decided to continue working with PC2 as it is completely batch-effect-free even though it is not ideal.
